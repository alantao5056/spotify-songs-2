...
draws <-
  ...
  mutate(
    valence = case_when(
      valence == 0 ~ "Not Positive - 0",
      valence == 25 ~ "Slightly Positive - 25",
      valence == 50 ~ "Moderately Positive - 50",
      valence == 75 ~ "Very Positive - 75",
      valence == 100 ~ "Extremely Positive - 100"
    ),
...
draws |>
  ...
  stat_slab(aes(fill = fct_reorder(valence, .epred, .desc = TRUE)), alpha = 0.7) +
  theme_bw() +
  labs(title = "Prediction of Popularity Given Genre and Valence",
       subtitle = "Assuming that all other variables are set to the average",
       x = "Popularity",
       y = "Genre",
       fill = "Valence")

ggsave("src/plots/valence.png", width = 8, height = 6)
